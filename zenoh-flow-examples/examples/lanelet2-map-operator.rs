...
static LINK_ID_OUTPUT_STR: &str = "Config";
...
impl Lanelet2MapOperator {
    ...
    fn run(_ctx: ZFContext, mut inputs: ZFInput) -> RunResult {
        let mut result = HashMap::<ZFLinkId, Arc<dyn DataTrait>>::with_capacity(1);

        let config = get_input!(ZFString, String::from(LINK_ID_INPUT_STR), inputs)?.clone();
